...
export const ChatInput: FC<ChatInputProps> = ({}) => {
  ...
  const checkValid = (callback: any) => {
    ...
    if (!profile) {
      ...
      } else if (parseInt(questionCount) < freeQuestions) {
        localStorage.setItem("questionCount", (parseInt(questionCount) + 1).toString())
        toast.info(t(`${freeQuestions - parseInt(questionCount) - 1} questions remaining.`))
      } else {
        toast.error(t("You need to log in to continue asking questions."), {
          action: {
            label: "Go to Login",
